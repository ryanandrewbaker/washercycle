...
def test_missing_power_returns_none():
    n = InputNormalizer()
    assert n.normalize_power("sensor.power", "unavailable", datetime.now(timezone.utc)) is None


def test_negative_power_rejected():
    n = InputNormalizer()
    result = n.normalize_power("sensor.power", "-5", datetime.now(timezone.utc))
...
def test_unknown_not_treated_as_zero():
    n = InputNormalizer()
    result = n.normalize_power("sensor.power", "unknown", datetime.now(timezone.utc))
    assert result is None
...
def test_energy_reset_detected():
    ...
    ts2 = datetime(2026, 1, 1, 10, 1, tzinfo=timezone.utc)
